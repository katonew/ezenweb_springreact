...
/*
// 1. 예제 훅[ useState() ] 를 안 썼을때
    // return : 렌더링 될 HTMl 반환 해주는 곳 [ 화면에 표시할 코드 ]
    // 컴포넌트 당 1번씩 실행 didMount
...
export default function Hook1(props){
    ...
    return (<>
        <div>
            ...
            <button onClick={countHandler}>증가</button>
        </div>

    </>)
}

// 2. 예제 훅 [ useState() ] 를 사용했을때
    // didMount -->
    // 상태변수 상태변경 -> set~~~ => didUpdate
    // 상태변경시 주소값이 변경 되어야 인식
export default function Hook1(props){
    console.log('Hook1 컴포넌트 실행')
    let[count,setCount] = useState(0);

    const countHandler = ()=>{
        console.log('countHandler 실행')
        setCount(count+1)
        console.log('count : ' + count)
    }
    return (<>
...
export default function Hook1(props){
    ...
    // 1. useEffect() 함수
        /*
            ...
            mount : 첫 생성, update : 변경, unmount : 제거
            1. useEffect( ()=>{} );
    ...
    useEffect( ()=>{
        ...
        return () => {console.log('컴포넌트 제거');} // unmount
    })
